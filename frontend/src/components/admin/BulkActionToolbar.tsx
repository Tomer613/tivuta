...
export default function BulkActionToolbar({
    ...
    return (
        <div className="flex flex-wrap items-center gap-3 mb-4 bg-[#0e1628] border border-[#d4af37]/30 rounded-2xl px-5 py-3">
            ...
            <select value={bulkAction} onChange={(e) => onBulkActionChange(e.target.value)} className="bg-[#111a2f] border border-[#d4af37]/20 rounded-xl px-3 py-1.5 text-xs text-[#f0e6d3]">
                <option value="">בחר פעולה...</option>
                <option value="set_status">שינוי סטטוס</option>
                <option value="assign">הקצאה לנציג</option>
            </select>
            {bulkAction === 'set_status' && (
                <select value={bulkValue} onChange={(e) => onBulkValueChange(e.target.value)} className="bg-[#111a2f] border border-[#d4af37]/20 rounded-xl px-3 py-1.5 text-xs text-[#f0e6d3]">
            ...
            <button onClick={onExecute} disabled={loading || !bulkAction} className="bg-[#d4af37] text-[#080d1f] px-4 py-1.5 rounded-xl text-xs font-black disabled:opacity-50 flex items-center gap-1.5">
