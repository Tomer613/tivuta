...
import { getCategoryBySlug, getCategoryItems } from '@/lib/api';
        ...
        <main className="min-h-screen bg-slate-50">
            ...
            <section className="bg-white border-b border-slate-200 pt-32 pb-16 px-6">
                <div className="max-w-7xl mx-auto relative">
                    <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-start">
                        {/* Back Button now part of the flex flow, appearing to the right of the icon in RTL */}
                ...
                <section className="max-w-7xl mx-auto py-12 px-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        {category.sub_categories.map((sub: any) => (
                            ...
                            >
                                {sub[`name_${locale as 'he' | 'en' | 'fr' | 'yi'}`] || sub.name_he}
                            </Link>
                        ))}
                    </div>
                </section>
            ...
            <section className="max-w-7xl mx-auto py-12 px-6">
                <h2 className="text-3xl font-black text-slate-900 mb-12 text-start">
                    {activeSubSlug 
                        ? category.sub_categories.find((s: any) => s.slug === activeSubSlug)?.[`name_${locale as 'he' | 'en' | 'fr' | 'yi'}`] || t.all_items
                        : t.all_items
                    }
                </h2>
                ...
                {filteredItems.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-10">
                        {filteredItems.map((item: any) => (
                            <ItemCard key={item.id} item={item} locale={locale as any} />
                        ))}
                    </div>
                ) : (
