...
test('cart checkout preserves each product\'s own quantity', async ({ page }) => {
    ...
    const tileA = page.getByTestId(`product-tile-${PRODUCT_A}`);
    const tileB = page.getByTestId(`product-tile-${PRODUCT_B}`);
