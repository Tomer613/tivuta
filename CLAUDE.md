...
| Backend keep-warm | GitHub Actions cron (`.github/workflows/keepalive.yml`) | Pings `https://tivuta.onrender.com/` every 10 minutes (`*/10 * * * *`) so Render's free-tier backend doesn't spin down after ~15 min idle, avoiding a ~50s cold-start hit for the next real visitor. Runs independently of `deploy.yml` — this is why "Keep Backend Warm" shows up constantly in the Actions run list, separate from actual deploys. |
| Scheduled-workflow keepalive | GitHub Actions cron (`.github/workflows/prevent-deactivation.yml`) | Runs once a day (`0 0 * * *`), makes a trivial auto-commit (`gautamkrishnar/keepalive-workflow`) purely so GitHub doesn't auto-disable the repo's other scheduled cron workflows (`keepalive.yml`, `schedule.yml`, `prune-analytics.yml`) for 60 days of apparent inactivity — a housekeeping workflow, not a feature. |
