...
## Loyalty & Vendor Commission Program (Phases 1–3 — session 2026-07-18)

Full design plan: see `.claude/plans` history or ask for the "sparkling-swimming-puffin" plan — this section documents what's actually **built** so far (Phases 1–3 of a 5-phase rollout).
...
- **Commission settlement lifecycle, now fully wired**: `POST /admin/vendors/{id}/settlements` opens a period — sums *unsettled* confirmed transactions (`settlement_period_id IS NULL`) within `[period_start, period_end]` by `confirmed_at` and links them to the new period (a transaction can only ever belong to one period, so overlapping ranges can't double-count). `PATCH /admin/vendors/{id}/settlements/{period_id}/settle` marks it paid, decrements `Vendor.commission_owed_total` by the period total, and rejects settling an already-settled period. Vendors see their own history read-only via `GET /vendor/settlements`.
- **Admin vendors page** (`frontend/.../admin/vendors/page.tsx`) gained: commission/points rate % inputs in the create/edit form, a commission-owed column in the table, and a key-icon button opening a small modal to issue/reset portal login credentials (`adminSetVendorPortalAccess` in `lib/api.ts`).
...
- `backend/app/services/loyalty.py` — customer-number generation, setting lookup w/ fallback + write-side validation, points/commission computation, and the shared `validate_and_resolve_sale_inputs()`/`create_sale_transaction()` used by both reporting paths
...
- `backend/app/routers/vendors.py` — vendor CRUD, `portal-access`, settlement open/settle endpoints
